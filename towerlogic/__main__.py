"""Main entry point for the TowerLogic ttkbootstrap interface."""
...
from towerlogic.bot.worker import WorkerProcess
from towerlogic.emulators import EmulatorType
from towerlogic.emulators.adb import AdbController
from towerlogic.interface.enums import PRIMARY_JOB_TOGGLES, UIField
from towerlogic.interface.ui import TowerLogicUI, no_jobs_popup
from towerlogic.utils.caching import USER_SETTINGS_CACHE
...
def make_job_dictionary(values: dict[str, Any]) -> dict[str, Any]:
    ...
    job_dictionary[UIField.BS_INSTANCE_NAME.value] = values.get(UIField.BS_INSTANCE_NAME.value)
    ...
    job_dictionary[UIField.POLICY_TOGGLE.value] = values.get(UIField.POLICY_TOGGLE.value, False)
    job_dictionary[UIField.POLICY_EPSILON.value] = values.get(UIField.POLICY_EPSILON.value, 0.10)
    job_dictionary[UIField.POLICY_MODEL_PATH.value] = values.get(UIField.POLICY_MODEL_PATH.value)
    job_dictionary[UIField.POLICY_TRAIN_TOGGLE.value] = values.get(UIField.POLICY_TRAIN_TOGGLE.value, False)
    job_dictionary[UIField.POLICY_LR.value] = values.get(UIField.POLICY_LR.value, 0.0005)
    job_dictionary[UIField.POLICY_FREE_PLACEMENT.value] = values.get(UIField.POLICY_FREE_PLACEMENT.value, False)
    job_dictionary[UIField.POLICY_MIN_ELIXIR.value] = values.get(UIField.POLICY_MIN_ELIXIR.value, 4)
    job_dictionary[UIField.POLICY_ALLOW_HOLD.value] = values.get(UIField.POLICY_ALLOW_HOLD.value, True)
    job_dictionary[UIField.POLICY_DEBUG_DETECTION.value] = values.get(UIField.POLICY_DEBUG_DETECTION.value, False)
    job_dictionary[UIField.POLICY_DEBUG_OVERLAY.value] = values.get(UIField.POLICY_DEBUG_OVERLAY.value, False)
    job_dictionary[UIField.POLICY_DEBUG_HP_LOG.value] = values.get(UIField.POLICY_DEBUG_HP_LOG.value, False)
    job_dictionary[UIField.POLICY_DEBUG_HAND_CONF.value] = values.get(UIField.POLICY_DEBUG_HAND_CONF.value, False)
    job_dictionary[UIField.POLICY_STRICT_ELIXIR.value] = values.get(UIField.POLICY_STRICT_ELIXIR.value, False)
    job_dictionary[UIField.POLICY_SPELL_GATE.value] = values.get(UIField.POLICY_SPELL_GATE.value, False)
...
def load_settings(settings: dict[str, Any] | None, ui: TowerLogicUI) -> dict[str, Any] | None:
    ...
    if loaded:
        loaded = dict(loaded)
        theme_key = UIField.THEME_NAME.value
        dark_themes = {"darkly", "superhero", "cyborg", "solar"}
        if str(loaded.get(theme_key, "")).strip().lower() in dark_themes or theme_key not in loaded:
            loaded[theme_key] = ui.DEFAULT_THEME
        ui.set_all_values(loaded)
        save_current_settings(loaded)
    return loaded
...
def update_layout(ui: TowerLogicUI, logger: Logger) -> None:
    ...
    if stats and isinstance(stats.get("log_tail"), str):
        ui.append_log(stats["log_tail"])
    else:
