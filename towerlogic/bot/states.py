...
from towerlogic.bot.card_mastery_state import card_mastery_state
from towerlogic.bot.deck_cycle import select_deck_state
from towerlogic.bot.deck_randomization import randomize_deck_state
from towerlogic.bot.fight import (
    do_2v2_fight_state,
    do_fight_state,
    end_fight_state,
    start_fight,
)
from towerlogic.bot.nav import check_if_battle_mode_is_selected, select_mode
from towerlogic.bot.upgrade_state import upgrade_cards_state
from towerlogic.interface.enums import UIField
from towerlogic.utils.caching import (
...
def state_tree(
    ...
    if state == "1v1_fight":
        ...
        policy_cfg = {
            "enabled": job_list.get(UIField.POLICY_TOGGLE.value, False),
            "epsilon": job_list.get(UIField.POLICY_EPSILON.value, 0.1),
            "model_path": job_list.get(UIField.POLICY_MODEL_PATH.value),
            "train": job_list.get(UIField.POLICY_TRAIN_TOGGLE.value, False),
            "lr": job_list.get(UIField.POLICY_LR.value, 5e-4),
            "free_placement": job_list.get(UIField.POLICY_FREE_PLACEMENT.value, False),
            "min_elixir": job_list.get(UIField.POLICY_MIN_ELIXIR.value, 4),
            "allow_hold": job_list.get(UIField.POLICY_ALLOW_HOLD.value, True),
            "strict_elixir": job_list.get(UIField.POLICY_STRICT_ELIXIR.value, False),
            "spell_gate": job_list.get(UIField.POLICY_SPELL_GATE.value, False),
            "debug_detection": job_list.get(UIField.POLICY_DEBUG_DETECTION.value, False),
            "debug_overlay": job_list.get(UIField.POLICY_DEBUG_OVERLAY.value, False),
            "debug_hp_log": job_list.get(UIField.POLICY_DEBUG_HP_LOG.value, False),
            "debug_hand_conf": job_list.get(UIField.POLICY_DEBUG_HAND_CONF.value, False),
        }
        if (
            do_fight_state(
                emulator,
