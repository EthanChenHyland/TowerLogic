...
from towerlogic.bot.states import StateHistory, StateOrder, state_tree
from towerlogic.emulators import EmulatorType, get_emulator_registry
from towerlogic.utils.cancellation import CancellationToken
    ...
    def _setup_emulator(self, jobs: dict[str, Any], logger: ProcessLogger):
        ...
        try:
            if emulator_selection == EmulatorType.GOOGLE_PLAY:
                ...
                bs_instance_name = jobs.get("bs_instance_name")
                return controller_class(
                    logger=logger,
                    render_settings=render_settings,
                    instance_name=bs_instance_name,
                )

            elif emulator_selection == EmulatorType.MEMU:
                print("Creating MEmu emulator")
                from towerlogic.emulators.memu import verify_memu_installation
